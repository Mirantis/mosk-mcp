"""Pytest fixtures for node lifecycle tool tests.

This module provides common fixtures for testing node lifecycle tools
...
def admin_context() -> UserContext:
    ...
    All node lifecycle tools (delete_machine, apply_machine,
    create_maintenance_request) require authentication and are decorated with
...
def operator_context() -> UserContext:
    ...
    Operators can perform non-destructive operations like:
    - create_maintenance_request

    But NOT privileged operations like:
    - delete_machine
    - apply_machine
    """
    return UserContext(
...
def viewer_context() -> UserContext:
    ...
    Viewers should NOT be able to perform any node lifecycle operations.
